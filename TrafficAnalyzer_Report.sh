...
#                        ['count'] ['sortby='column] ['trimdb[='max_sqldb_size]]
#
#    TrafficAnalyzer
	...
	if [ -n "$CMDCOUNT" ];then	
		...
		sqlite3 $SQL_DATABASE "SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, cat_name, app_name, tx, rx FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;" | while IFS= read -r LINE
			do
				...
				printf '%b%-12d%-12d%b%-10s%b %-8s%b %-18s%-16s%b%-15s%b%-33s%b%-32s\n' "${cBBLU}$xERASE" "$RX" "$TX" "$COLOUR_DATE" "$DATE"  "$COLOUR_TIME" "$TIME" "$cBBLU" "$MAC" "$HOSTNAME" "$COLOUR_IP" "$IP" "$COLOUR_CAT" "$CAT" "$COLOUR_APP" "$APP"
